...
function onUseWeaponSkill(player, target, wsID)
	...
	local params = {};
	params.numHits = 2;
	params.ftp100 = 2.25; params.ftp200 = 2.65; params.ftp300 = 3;
	params.str_wsc = 0.2; params.dex_wsc = 0.6; params.vit_wsc = 0.0; params.agi_wsc = 0.0; params.int_wsc = 0.0; params.mnd_wsc = 0.0; params.chr_wsc = 0.0;
	...
	if ((player:getEquipID(SLOT_MAIN) == 18300) and (player:getMainJob() == JOB_DRG)) then
		if (damage > 0) then
			...
			elseif (player:getTP() >= 200 and player:getTP() < 300) then
				player:addStatusEffect(EFFECT_AFTERMATH, 6, 0, 120, 0, 7);
			elseif (player:getTP() == 300) then
				player:addStatusEffect(EFFECT_AFTERMATH, 6, 0, 180, 0, 7);
			end
		end
	end
