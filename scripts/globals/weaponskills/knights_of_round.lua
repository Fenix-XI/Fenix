...
function onUseWeaponSkill(player, target, wsID)
	...
	if ((player:getEquipID(SLOT_MAIN) == 18276) and (player:getMainJob() == JOB_RDM or JOB_PLD or JOB_BLU)) then
		if (damage > 0) then
			if (player:getTP() >= 100 and player:getTP() < 200) then
				player:addStatusEffect(EFFECT_AFTERMATH, -10, 0, 40, 0, 5);
			elseif (player:getTP() >= 200 and player:getTP() < 300) then
				player:addStatusEffect(EFFECT_AFTERMATH, -10, 0, 120, 0, 5);
			elseif (player:getTP() == 300) then
				player:addStatusEffect(EFFECT_AFTERMATH, -10, 0, 180, 0, 5);
			end
		end
	end
