...
function onUseWeaponSkill(player, target, wsID)
	...
	local params = {};
	params.numHits = 5;
	params.ftp100 = 1.0; params.ftp200 = 1.85; params.ftp300 = 2.75;
	params.str_wsc = 0.2; params.dex_wsc = 0.6; params.vit_wsc = 0.0; params.agi_wsc = 0.0; params.int_wsc = 0.0; params.mnd_wsc = 0.0; params.chr_wsc = 0.2;
	...
	if ((player:getEquipID(SLOT_MAIN) == 18288) and (player:getMainJob() == JOB_BST)) then
		if (damage > 0) then
			...
			elseif (player:getTP() >= 200 and player:getTP() < 300) then
				player:addStatusEffect(EFFECT_AFTERMATH, 10, 0, 120, 0, 4);
			elseif (player:getTP() == 300) then
				player:addStatusEffect(EFFECT_AFTERMATH, 10, 0, 180, 0, 4);
			end
		end
	end
