-- Variable TextID   Description text

-- General Texts
ITEM_CANNOT_BE_OBTAINED = 6379; -- You cannot obtain the item <item> come back again after sorting your inventory
          ITEM_OBTAINED = 6384; -- Obtained: <item>
           GIL_OBTAINED = 6385; -- Obtained <number> gil
       KEYITEM_OBTAINED = 6387; -- Obtained key item: <keyitem>
            HOMEPOINT_SET = 7716; -- Home point set!

-- Other dialogs
        SD_VERY_SMALL = 7579; -- The spatial displacement is very small.
         SD_HAS_GROWN = 7580; -- The spatial displacement has grown.
SPACE_SEEMS_DISTORTED = 7581; -- The space around you seems oddly distorted and disrupted.
             MONUMENT = 7588; -- Something has been engraved on this stone, but the message is too difficult to make out.

NOTHING_OUT_OF_ORDINARY = 7595 -- There is nothing out of the ordinary here.

-- conquest Base
CONQUEST_BASE = 7045; -- Tallying conquest results...
